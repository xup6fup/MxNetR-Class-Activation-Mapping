library(imager)
library(EBImage)   #Installed by bioconductor
library(magrittr)
library(mxnet)
...
pdf('F2.pdf', height = 18, width = 24)

par(mai = rep(0, 4), mfrow = c(3, 4))

for (i in 1:6) {
  
  #Read and resized the image
  
  img_path = paste0('image/', i, '.jpg')
  
  resized_img <- load.image(img_path) %>% resize(., 224, 224)
